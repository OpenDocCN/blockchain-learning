# 7.2 基于 Kafka 排序服务的实现

## 目标

1.  实现现基于 Kafka 集群的排序服务

## 任务实现

> 在上一节中，我们知道了 Hyperledger Fabric 的共识实现，现在我们使用容器实现一个基于 Kafka 提供排序服务的集群环境。在该网络环境中，我们使用 2 个 Org 组织，每个 Org 组织下各有 2 个 peer 节点，4 个 orderer 节点，后端使用 Kafka 集群实现排序服务，由 4 个 kafka 节点，3 个 zookeeper 节点组成。

### 7.2.1 指定 Kafka 实现排序服务

#### 7.2.1.1 配置 crypto-config.yaml
...
#### 7.2.2.1 配置 docker-compose-base.yaml
...
#### 7.2.2.2 配置 docker-compose-cli.yaml
...
可发现发三个 zookeeper 节点、四个 kafka 节点、四个 orderer 节点、四个 peer 节点都已经处于活动状态，如下图所示：

![集群容器](img/623cfaa0503389a24f1f953cee2efcdd.jpg)
...
![集群容器](img/5c96eff18ea2f7cfe5312df023a77ce1.jpg)

进入 kafka0 容器并进入 kafka HOME 目录
...
orderer 节点以集群的方式运行，在集群环境下，客户端将交易发送到任何一个 orderer 排序节点都可以，如下所示，指定 Orderer2 节点。
...
1.  之前从未使用过 Kafka / Zookeeper 集群，我想使用基于 Kafka 的 Ordering 服务。我该怎么办？

    如果之前从未使用过 Kafka，那么应该先从官方网站学习一下 [Kafka 快速入门指南](https://kafka.apache.org/quickstart)

2.  docker-compose-cli.yaml 配置文件有官方的配置示例吗？

    官方的配置示例请参阅 [端到端 CLI 示例](https://github.com/hyperledger/fabric/blob/master/examples/e2e_cli/docker-compose-e2e.yaml)

3.  为什么使用基于 Kafka 的集群服务中必须使用 ZooKeeper？
